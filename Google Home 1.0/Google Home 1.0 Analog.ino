...
#define AIO_USERNAME    "NicoGangi"            // Username
#define AIO_KEY         "4d6edb041a5a44809d3f5c33c9d5d5e5"   // Auth Key
...
Adafruit_MQTT_Subscribe _Pote = Adafruit_MQTT_Subscribe(&mqtt, AIO_USERNAME "/feeds/Pote");
...
void loop() {
  ...
  while ((subscription = mqtt.readSubscription(20000))) {
    ...
    if (subscription == &_Pote) {
      Serial.print(F("Got: "));
      Serial.println((char *)_Pote.lastread);
      int _Pote_State = atoi((char *)_Pote.lastread);

      _Pote_State = (_Pote_State * 255) / 100;
      analogWrite(Pote, _Pote_State);
    }
  }
